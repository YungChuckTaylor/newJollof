...
-- ----------------------------------------------------------------------------
--  0. USER VERIFIED-NESS COLUMNS (WP12)
-- ----------------------------------------------------------------------------
-- Users table tracking email and phone verification timestamps
SET @col_email_exist := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'users' AND column_name = 'email_verified_at');
SET @sql_email := IF(@col_email_exist = 0, 'ALTER TABLE `users` ADD COLUMN `email_verified_at` datetime DEFAULT NULL AFTER `kyc_verified`', 'SELECT 1');
PREPARE stmt_email FROM @sql_email;
EXECUTE stmt_email;
DEALLOCATE PREPARE stmt_email;

SET @col_phone_exist := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'users' AND column_name = 'phone_verified_at');
SET @sql_phone := IF(@col_phone_exist = 0, 'ALTER TABLE `users` ADD COLUMN `phone_verified_at` datetime DEFAULT NULL AFTER `email_verified_at`', 'SELECT 1');
