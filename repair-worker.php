<?php
/**
 * Jollof Living — repair worker. (Loaded by the tiny repair.php; you never
 * paste this file.) Two plans:
 *   plan A (default): fetch every file from GitHub, SHA-1 verified.
 *   plan B (?key=…&plan=b): GitHub blocked? Upload the branch ZIP instead —
 *          it is extracted and the same SHA-1 verification runs on it.
 * Never touches includes/config.php or jollof-automations/config.php.
 */
...
/** Copy $srcFile to $target if its sha1 matches $want. Returns message. */
function jl_place(string $srcFile, string $target, string $want, ?string $oldSize): string
{
    $body = @file_get_contents($srcFile);
    if ($body === false)                       return 'READ-FAIL';
    if (sha1($body) !== $want)                 return 'SHA-MISMATCH';
    if (!is_dir(dirname($target)) && !@mkdir(dirname($target), 0755, true)) return 'MKDIR-FAIL';
    if (@file_put_contents($target, $body, LOCK_EX) === false) return 'WRITE-FAIL';
    @chmod($target, 0644);
    @touch($target);
    return sprintf('OK (%6s -> %6s bytes)', $oldSize ?? '-', strlen($body));
}
...
echo "=== Jollof repair worker @ " . COMMIT . " (plan " . ($planB ? 'B: zip upload' : 'A: github fetch') . ") ===\n\n";
echo 'folder : ' . __DIR__ . "\n";
echo 'PHP    : ' . PHP_VERSION . "\n";
echo 'config : ' . (is_file(__DIR__ . '/includes/config.php') ? "present (untouched)\n" : "MISSING!\n");
echo 'disk   : ' . round((float) @disk_free_space(__DIR__) / 1048576) . " MB free\n\n";

/* ============================================================ plan A */
if (!$planB) {
    echo "--- fetching " . count($MANIFEST) . " files from GitHub ---\n";
    foreach ($MANIFEST as $path => $want) {
        $target = __DIR__ . '/' . $path;
        ...
        if (!is_dir(dirname($target)) && !@mkdir(dirname($target), 0755, true)) { echo "MKDIR-FAIL   $path\n"; $failed[] = $path; continue; }
        $old = is_file($target) ? (string) filesize($target) : '-';
        if (@file_put_contents($target, $body, LOCK_EX) === false) { echo "WRITE-FAIL   $path\n"; $failed[] = $path; continue; }
        if (sha1($body) !== $want) { echo "SHA-MISMATCH $path\n"; $failed[] = $path; continue; }
        @chmod($target, 0644);
        @touch($target);
        $ok++;
        echo sprintf("OK  %-52s %8s -> %6s\n", $path, $old, strlen($body));
    }
}
...
if ($planB) {
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
        echo "GitHub is not reachable from this server, so:\n";
        echo " 1. On YOUR computer, download:  https://github.com/YungChuckTaylor/newJollof/archive/refs/heads/arena/01a0d3cf-newjollof.zip\n";
        echo " 2. Below, pick that ZIP and press Repair.\n\n";
        echo '<form method="post" enctype="multipart/form-data" style="font-family:sans-serif">'
           . '<input type="hidden" name="key" value="' . htmlspecialchars($_GET['key']) . '">'
           . '<input type="file" name="zip" accept=".zip" required> '
           . '<button type="submit">Repair from ZIP</button></form>';
        exit;
    }
    ...
    if (!class_exists('ZipArchive')) {
        exit('ZipArchive missing on this host - ask HostGator support to enable the zip extension, or paste the files manually.');
    }
    $tmpZip = tempnam(sys_get_temp_dir(), 'jlz');
    @move_uploaded_file($_FILES['zip']['tmp_name'], $tmpZip);
    $za = new ZipArchive();
    if ($za->open($tmpZip) !== true) exit('Cannot open the ZIP.');
    $tmpDir = sys_get_temp_dir() . '/jl_extract_' . getmypid();
    @mkdir($tmpDir, 0755, true);
    $za->extractTo($tmpDir);
    $za->close();
    // strip the single top-level folder the GitHub zip adds
    $root = $tmpDir;
    $entries = array_diff(scandir($tmpDir) ?: [], ['.', '..']);
    if (count($entries) === 1) {
        $root = $tmpDir . '/' . reset($entries);
    }
    echo "--- extracting uploaded zip into place (" . count($MANIFEST) . " files) ---\n";
    foreach ($MANIFEST as $path => $want) {
        $srcFile = $root . '/' . $path;
        $res = is_file($srcFile)
            ? jl_place($srcFile, __DIR__ . '/' . $path, $want, is_file(__DIR__ . '/' . $path) ? (string) filesize(__DIR__ . '/' . $path) : null)
            : 'NOT-IN-ZIP';
        if (strncmp($res, 'OK', 2) === 0) { $ok++; echo "OK  $path  $res\n"; }
        else { echo str_pad($res, 13) . " $path\n"; $failed[] = $path; }
    }
    // health/status pages from the zip too
    foreach (['health.php', 'status.php'] as $name) {
        $srcFile = $root . '/' . 'health.php';
        if (is_file($srcFile) && @copy($srcFile, __DIR__ . '/' . $name)) { @chmod(__DIR__ . '/' . $name, 0644); $ok++; echo "OK  $name\n"; }
    }
}

/* ============================================== diagnostics + finish */
if (!is_file(__DIR__ . '/health.php') && is_file(__DIR__ . '/includes/view.php')) {
    echo "NOTE: health.php not written (fetch it later).\n";
}
if (function_exists('opcache_reset')) { @opcache_reset(); echo "\nopcache: cleared\n"; }

echo "\n=== RESULT: $ok written, " . count($failed) . " failed ===\n";
if ($failed) {
    echo "Failed: " . implode(', ', array_slice($failed, 0, 20)) . (count($failed) > 20 ? ' …' : '') . "\n";
    if (!$planB) {
        $key = htmlspecialchars($_GET['key']);
        echo "\nIf many FETCH-FAILs: this server cannot reach GitHub.\n";
        echo "Switch to plan B (upload the ZIP yourself):\n";
        echo "  repair.php?key=$key&plan=b\n";
    }
}

echo "\n--- test now ---\n";
echo " 1. homepage (hard refresh)      ../\n";
echo " 2. diagnostics                  health.php  (or status.php)\n";
echo " 3. admin -> install/migrate.php\n";
echo " 4. sitemap                      ../sitemap.xml\n";
echo "\nDelete repair.php + repair-worker.php + health.php + status.php when done.\n";
