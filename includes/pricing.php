...
final class BookingService
{
    ...
    public static function transition(string $ref, string $action, ?int $actingUserId = null): array
    {
        ...
        // Dispatch customer notification email on lifecycle transitions (WP10 / WP11)
        Mailer::bookingStatusUpdate($b, $action, $step['msg']);
        if (!empty($b['user_id'])) {
            Repo::notify((int) $b['user_id'], $step['msg'], 'Stay: ' . ($b['property_name'] ?? $b['ref']) . ' (' . $b['checkin'] . ')', 'calendar');
        }
