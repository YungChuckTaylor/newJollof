...
final class VerifyService
{
    ...
    /**
     * Create a cryptographic password reset token & 6-digit backup code (WP12 / F16).
     */
    public static function createPasswordResetToken(int $userId): array
    {
        self::ensureSchema();
        ...
        $tokenHash = hash('sha256', $rawToken);
        $codeHash  = hash('sha256', $rawCode);
        $expiresAt = date('Y-m-d H:i:s', time() + 3600); // 60-minute expiry

        // Invalidate older unused reset tokens
        try {
            DB::run('UPDATE verify_tokens SET consumed_at = ? WHERE user_id = ? AND purpose = "password_reset" AND consumed_at IS NULL', [
                date('Y-m-d H:i:s'),
                $userId,
            ]);
        } catch (Throwable $e) {
        ...
        $resetUrl = $siteUrl . '/reset.php?token=' . urlencode($rawToken);
    ...
    /**
     * Initiate password reset challenge and send email (WP12 / F16).
     */
    public static function sendPasswordReset(string $email): array
    {
        ...
        $user = DB::row('SELECT id, name, email, status FROM users WHERE email = ?', [$email]);
        if (!$user) {
            // Constant-time success response to avoid email enumeration
            return [
                'ok'      => true,
                'message' => 'If an account exists with that email address, a password recovery link has been sent.',
            ];
        }

        if (Auth::isBlocked($user)) {
            return ['ok' => false, 'message' => 'This account is suspended. Please contact concierge support.'];
        }

        // 60-second cooldown per user
        $recent = DB::row(
            'SELECT created_at FROM verify_tokens WHERE user_id = ? AND purpose = "password_reset" ORDER BY id DESC LIMIT 1',
            [(int) $user['id']]
        );
        if ($recent && (time() - strtotime((string) $recent['created_at'])) < 60) {
            $wait = 60 - (time() - strtotime((string) $recent['created_at']));
            return ['ok' => false, 'message' => "Please wait {$wait} seconds before requesting another recovery email."];
        }

        $t = self::createPasswordResetToken((int) $user['id']);
        ...
        audit((string) $user['email'], 'Password reset email ' . ($sent ? 'dispatched' : 'delivery failed'), $sent ? 'ok' : 'warn');

        return [
            'ok'      => true,
            'message' => $sent
                ? 'A password recovery link has been sent to your email. Please check your inbox and spam folder.'
                : 'Could not send the password recovery email. Please check SMTP settings.',
            'sent'    => $sent,
        ];
    }

    /**
     * Complete password reset using cryptographic token and revoke existing sessions (WP12 / F16 / F22).
     */
    public static function resetPasswordWithToken(string $rawToken, string $newPassword): array
    {
        self::ensureSchema();
        $rawToken = trim($rawToken);
        if ($rawToken === '') {
            return ['ok' => false, 'message' => 'Invalid or missing recovery token.'];
        }
        if (mb_strlen($newPassword) < 8) {
            return ['ok' => false, 'message' => 'Your password needs at least 8 characters.'];
        }

        $tokenHash = hash('sha256', $rawToken);
        $row = DB::row(
            'SELECT * FROM verify_tokens WHERE token_hash = ? AND purpose = "password_reset" ORDER BY id DESC LIMIT 1',
            [$tokenHash]
        );

        if (!$row) {
            return ['ok' => false, 'message' => 'This recovery link is invalid or does not exist.'];
        }
        if (!empty($row['consumed_at'])) {
            return ['ok' => false, 'message' => 'This recovery link has already been used. Please request a new one.'];
        }
        if (strtotime((string) $row['expires_at']) < time()) {
            return ['ok' => false, 'message' => 'This recovery link has expired. Please request a new one.'];
        }
        ...
        // Revoke all previous sessions (force logout everywhere, F16 / F22)
        Auth::revokeSessions($userId);

        audit((string) $user['email'], 'Password reset successfully via email recovery link', 'ok');
        Repo::notify($userId, 'Password changed 🔒', 'Your password was successfully updated.', 'shield');
