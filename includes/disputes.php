...
final class DisputeService
{
    ...
    public static function open(array $in, ?int $userId): array
    {
        ...
        // Alert management/operations team of new dispute
        Mailer::adminNotice(
            'New dispute opened · ' . $ref,
            "Dispute reference: {$ref}\nClaimant: {$name} ({$email})\nSubject: {$subject}\nSLA window: {$sla} hours\nView case: " . url('admin.php?tab=disputes&ref=' . urlencode($ref))
        );
    ...
    public static function resolve(string $ref, array $in, array $actor): array
    {
        ...
        if ($d['counterparty_id']) {
            ...
            $cpUser = Repo::userRow((int) $d['counterparty_id']);
            if ($cpUser && !empty($cpUser['email']) && is_email((string) $cpUser['email'])) {
                Mailer::send((string) $cpUser['email'], 'Dispute decision · ' . $d['ref'],
                    '<p>The mediation team has issued a decision on dispute <b>' . e((string) $d['ref']) . '</b>.</p>'
                    . '<p>Outcome: <b>' . e(self::OUTCOMES[$outcome]) . '</b>' . ($refund > 0 ? ' — Refund: ' . e(money($refund)) : '') . '</p>'
                    . '<p>' . nl2br(e($note)) . '</p>'
                    . '<p><a href="' . e(url('host-dashboard.php')) . '">View your dashboard</a></p>');
            }
        }
